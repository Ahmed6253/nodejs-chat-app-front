...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const errorsToShow = validateForm();
    if (Object.keys(errorsToShow).length > 0) {
      setErrors(errorsToShow);
      for (const [_, value] of Object.entries(errorsToShow)) {
        toast.error(value);
      }
      return;
    }
      ...
      <div className="flex flex-col justify-center items-center p-6 sm:p-12">
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              className="btn bg-primary text-primary-content hover:bg-primary/90 w-full"
